...
async def project_info(ctx):
    user_id = ctx.author.id
    await ctx.send('Masukkan nama proyek yang ingin ditampilkan informasinya')

    def check(msg):
        return msg.author == ctx.author and msg.channel == ctx.channel

    project_name = await bot.wait_for('message', check=check)
    info = manager.get_project_info(user_id, project_name.content)
    if not info:
        await ctx.send('Proyek tidak ditemukan atau bukan milik Anda.')
        return

    # info returns [(project_name, description, url, status_name)]
    project_name, description, url, status = info[0]
    skills = manager.get_project_skills(project_name)
    text = f"Nama: {project_name}\nDeskripsi: {description or '-'}\nLink: {url or '-'}\nStatus: {status or '-'}\nKeterampilan: {skills or '-'}"
    await ctx.send(text)
